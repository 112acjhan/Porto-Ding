...
export default function InventoryBaseView() {
  ...
  // ✅ Uses real Google Sheets data with Number() to handle string values from sheet
  const totalValue = inventory
    .reduce((acc, curr) => acc + (Number(curr.stock_level) * Number(curr.unit_price)), 0)
    .toFixed(2);

  const lowStockCount = inventory.filter(i =>
    Number(i.stock_level) < Number(i.reorder_point)
  ).length;
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      {/* Stats Cards — all computed from real Google Sheets data */}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-brand-bg/30 text-[10px] uppercase tracking-widest text-brand-text-dim font-bold">
                ...
                <th className="px-6 py-4 border-b border-brand-border">Reorder Point</th>
                <th className="px-6 py-4 border-b border-brand-border">Price (RM)</th>
            ...
            <tbody className="text-xs">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center py-10 text-brand-text-dim">
                    No items found.
                  </td>
                </tr>
              ) : filteredItems.map((item) => (
                <tr key={item.item_id} className="hover:bg-brand-bg/20 transition-colors group">
                  <td className="px-6 py-4 border-b border-brand-border font-mono text-brand-text-dim">
                    {item.item_id}
                  </td>
                  <td className="px-6 py-4 border-b border-brand-border font-bold">
                    {item.item_name}
                  </td>
                  <td className="px-6 py-4 border-b border-brand-border font-black">
                    {Number(item.stock_level).toLocaleString()}
                  </td>
                  <td className="px-6 py-4 border-b border-brand-border text-brand-text-dim">
                    {Number(item.reorder_point).toLocaleString()}
                  </td>
                  <td className="px-6 py-4 border-b border-brand-border text-brand-text-dim">
                    RM {Number(item.unit_price).toFixed(2)}
                  </td>
                  <td className="px-6 py-4 border-b border-brand-border">
                    {Number(item.stock_level) < Number(item.reorder_point) ? (
                      <span className="px-2 py-1 bg-rose-500/10 text-rose-500 text-[9px] font-black rounded">
                        LOW STOCK
                      </span>
                    ) : (
                      <span className="px-2 py-1 bg-brand-success/10 text-brand-success text-[9px] font-black rounded">
                        OPTIMAL
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-4 border-b border-brand-border text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="p-2 hover:bg-brand-bg rounded text-brand-text-dim hover:text-brand-accent transition-colors"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
                        onClick={() => handleDelete(item.item_id)}
                        className="p-2 hover:bg-brand-bg rounded text-brand-text-dim hover:text-rose-500 transition-colors"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-brand-surface border border-brand-border rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="text-[10px] uppercase font-bold text-brand-text-dim">
                  Item ID (Unique)
                </label>
                <input
                  disabled={isEditing}
                  required
                  className="w-full bg-brand-bg border border-brand-border rounded-lg px-4 py-2 mt-1 text-sm outline-none focus:border-brand-accent disabled:opacity-50"
              ...
              <div>
                ...
                <input
                  required
                  className="w-full bg-brand-bg border border-brand-border rounded-lg px-4 py-2 mt-1 text-sm outline-none focus:border-brand-accent"
              ...
              <button
                type="submit"
                className="w-full py-3 bg-brand-accent text-white rounded-lg font-bold mt-4 hover:brightness-110 transition-all"
              >
...
function StatCard({ label, value, icon: Icon, color }: any) {
  return (
    <div className="bg-brand-surface p-5 rounded-xl border border-brand-border flex items-center gap-4">
      <div className={cn("p-3 rounded-xl bg-brand-bg", color)}>
        <Icon size={24} />
      </div>
      <div>
        <p className="text-[10px] font-bold text-brand-text-dim uppercase tracking-widest leading-none mb-1">
          {label}
        </p>
